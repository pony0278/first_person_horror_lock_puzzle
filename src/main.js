...
/* ═══════════════════════════════════════════════════════════
   程序化材質
   ═══════════════════════════════════════════════════════════ */
let rngTex = mulberry32(CFG.world.seed);

function makeHeightField(size, oct = 5) {
...
const S = 256, field = makeHeightField(S);
const tMap = texFrom(field, S, 0.72, 1.0),
      tRgh = texFrom(field, S, 0.62, 0.98),
      tNrm = normalFrom(field, S, 2.2);

function concrete(tint, rx, ry) {
...
const matWall = concrete(0x8e9095, 2, 2.4);
const matFloor = concrete(0x6a6c70, 2, 3);
const matCeil = concrete(0x76787c, 2, 3);
const matDoor = concrete(0x55585d, 1, 1.4);
const matMetal = new THREE.MeshStandardMaterial({ color: 0x4e5257, roughness: .5, metalness: .45 });
const matDark = new THREE.MeshBasicMaterial({ color: 0x000000 });
...
const DW = W * 0.86, DH = 2.15, FRAME = 0.13;
...
const LEAF_Z = -0.07;
const panelMesh = new THREE.Mesh(boxGeo, matDoor);
...
/* ── 鎖組：固定座（含基準刻痕）＋ 會轉的鎖芯 ─────────── */
const LOCK_Y = 1.30, LOCK_X = 0.38;    // 靠把手側，但保持在 55° FOV 內
...
const plate = new THREE.Mesh(cylGeo, matPlate);
...
const refMark = new THREE.Mesh(boxGeo, new THREE.MeshStandardMaterial({ color: 0xb9c0c8, roughness: .5 }));
...
/* ═══════════════════════════════════════════════════════════
   ...
   人形骨架 + 配錯的零件（反關節後腿、獸吻、無鼻、全黑濕眼）。
   凹陷靠假 AO 頂點色壓暗 —— 凸面只有可愛，凹面才有恐怖。
   ═══════════════════════════════════════════════════════════ */
const MP = {
  // 比例 —— 每一項都是「離正常多遠」的槓桿
...
let skinMat, socketMat, shineMat, blackEyeMat, toothMat;
...
/* ═══════════════════════════════════════════════════════════
   ...
   才有恐怖，凸面只有可愛。
   ═══════════════════════════════════════════════════════════ */
let torsoRef = null, headRef = null;
const SIDES = 12;

/* 由一串環（中心、橢圓半徑、頂點暗度）生成連續管狀網格 */
function tubeGeometry(rings, capTop, capBot) {
  const pos = [], col = [], idx = [];
...
/* 沿一條路徑生成錐狀肢體 */
function limbRings(from, to, r0, r1, bend, bendAxis) {
  const rings = [], N = 7;
...
const monster = buildMonsterMesh();
scene.add(monster);
/* 微生命的錨點：軀幹（呼吸）與頭（歪斜擺動）。
   舊的 children[0]/[1] 索引已失效，改用具名參考。 */
const monsterTorso = torsoRef, monsterHead = headRef;
/* 量測頭部高度：貼臉時要讓臉落在相機視線高度，
   否則 0.8m 距離下臉會被切在畫面上緣外（只看到胸口）。 */
const MONSTER_HEAD_Y = (() => {
  monster.updateMatrixWorld(true);
  return monsterHead.getWorldPosition(new THREE.Vector3()).y;
})();
const FACE_DROP = 1.62 - MONSTER_HEAD_Y + 0.02;            // 俯身湊近的下沉量（相機眼高 1.62）


/* 壞掉的緊急照明：現在是純氛圍與構圖用光源，
   怪物停在燈下那一站時會被它背光打成剪影。 */
const lamp = new THREE.PointLight(CFG.lamp.color, CFG.lamp.intensity, 34, 1.15);
...
function makeSeepMaterial(seed) {
...
const ST = { index: 0, z: 0, targetZ: 0, moveT: 0, pendingJump: false,
...
function makePaintTexture(order) {
  const w = 512, h = 320;
...
const paintPlane = new THREE.Mesh(planeGeo, paintMat);
...
const markerMat = new THREE.MeshBasicMaterial({ color: 0xd8b25a });
const marker = new THREE.Mesh(boxGeo, markerMat);
marker.scale.set(0.05, 0.05, 0.03);
marker.position.set(-CFG.world.corridorW / 2 + 0.05, CFG.paint.baseY + 0.75, CFG.paint.wallZ);
scene.add(marker);
const markerLight = new THREE.PointLight(0xd8b25a, 0, 2.6, 1.8);
markerLight.position.copy(marker.position).x += 0.12;
scene.add(markerLight);

/* 開場序列：奔跑 → 撞門 → 壓門把拉不開 → 工具入孔 → 交出控制權 */
const intro = { active: false, phase: 'run', t: 0, z: 0, bobPhase: 0,
                bobY: 0, roll: 0, arriveF: 1, beeped: false, th1: false, th2: false };

/* ── 光 ─────────────────────────────────────────────── */
const flash3d = new THREE.SpotLight(0xfff2d8, CFG.light.near.intensity, CFG.light.distance,
...
const fill = new THREE.PointLight(0x9db0c8, CFG.light.fill, 3.2, 1.6);
camera.add(fill);
...
const vig = new THREE.Mesh(planeGeo, vigMat);
...
const labSkinMat = new THREE.MeshStandardMaterial({
  color: 0xa08a78, roughness: 0.8, metalness: 0, side: THREE.DoubleSide,
});
const GAME_HAND_SCALE = 0.151;   // lab 尺寸 → 真實手長約 19cm
...
const handR = createLabHand(+1), handL = createLabHand(-1);
...
const hd = { on: false, fix: { rx: 0, ry: 0, rz: 0 } };
const $hdbg = document.getElementById('hdbg');
const $hdPose = document.getElementById('hdPose');
...
function hPlace(hand, P, swingPhase, pressF, extra) {
  ...
  const jx = handShake > 0 ? (Math.random()-0.5)*0.012 : 0;
  const jy = handShake > 0 ? (Math.random()-0.5)*0.012 : 0;
...
const look = { yaw: 0, target: 0, holding: false };
const $pins = document.getElementById('pins');
const $seq = document.getElementById('seq');
const $panel = document.getElementById('panel');
const $fade = document.getElementById('fade');
const $dev = document.getElementById('dev');
let tracks = [], sel = 0, devOn = false;

let cutCanvas = null, cutCtx = null;
const flashUntil = [];        // 每個栓位的錯誤閃爍到期時間
...
/* 垂直圓柱感：左暗、中亮帶、右暗 */
function cylRect(ctx, x0, y0, x1, y1, base, hi, lo) {
  const w = x1 - x0;
...
const pick = { x: 0, lift: 0 };

function drawCutaway(tint = 0) {
  if (!cutCtx) return;
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (i === sel && pick.lift > 0 && (st === 'idle' || st === 'partial')) {
...
$pins.addEventListener('pointermove', e => {
  ...
  if (idx !== sel) { sel = idx; drag.acted = false; drag.y = e.clientY; renderPins(); }
  const dy = drag.y - e.clientY;
  pick.lift = drag.acted ? 0 : Math.max(0, Math.min(1, dy / 20));   // 工具頂栓的即時行程
  if (!drag.acted && dy >  20) { doPush(sel);    drag.acted = true; pick.lift = 0; }
  if (!drag.acted && dy < -20) { doRelease(sel); drag.acted = true; pick.lift = 0; }
});
$pins.addEventListener('pointerup', () => {
  if (drag && !drag.acted) doPush(sel);          // 單點 = 頂針
...
const stopLook = () => { look.holding = false; look.target = 0; };
...
addEventListener('keydown', e => {
  ...
  if (e.key === 'd') { devOn = !devOn; $dev.style.display = devOn ? 'block' : 'none'; }
  if (e.key === 'h') {
    hd.on = !hd.on;
...
function tick() {
  ...
  // 撬針在孔內的行程：橫向選栓（幅度很小，不能離開孔）＋ 上頂
  {
    ...
    const tx = (sel / n - 0.5) * 0.034;                 // 孔寬之內
    const st = R.lock.pins[sel];
  ...
  seepUni.uTime.value += dt / Math.max(0.05, timeScale);   // 真實時間，不吃慢動作
