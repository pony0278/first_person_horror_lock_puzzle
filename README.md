...
目前階段：**F2.4 門 3 凝視牽制與最終追殺**（門 1、門 2 玩法已接通；門 3 已啟用守恆轉液、雙門閂、首次爆管、誠實三向威脅、分階段有限凝視與開門後最後逃脫）
...
| Job | 內容 | 擋發布嗎 |
| --- | --- | --- |
| **建置與發布** | `npm run check`（型別、相依分層、120 個單元測試）→ 建置單檔 → 產物大小門檻 → 發布 | 是 —— 但這些都是純 Node，兩秒跑完且完全確定性 |
...
src/
  ...
  logic/                  ── 純邏輯，strict TypeScript，不碰 DOM 與 Three.js ──
    ...
    door3-threat.ts         門 3 三向提示、距離階段與致死時序
    door3-pursuit.ts        門 3 凝視預算、開門追殺與跨門判定
  render/                 ── 場景建構與繪製 ──
    materials.js            程序化材質
    scene.js                走廊、門、鎖芯、工具
    monster.js              怪物網格
...
門 3 已完成淹水十字泵房、壓力路由與 F2.4 最終逃脫：Door 2 門後是 16m 積水連接廊，玩家連續奔跑到低矮工作檯，再以 6／4／3 容量守恆轉液完成雙門閂。第一次合法轉移會觸發爆管與四秒濕視野；取回操作後，同一隻怪物依後→左→右循環換道、先用鐵鏈／水波／熄燈誠實提示，再逐段顯形接近。玩家只能在對準可見怪物時為每階段牽制 1.60 秒；用完後的 0.18 秒整室燈閃會遮住換道並重播完整新方向提示。拉下總閘時同一隻怪物從當前方向與距離進入開門追殺；第五階段需用滿最終凝視，才可在 0.42m 捕捉距離前跨過門檻並觸發「極限逃脫」。Performance Stabilization v2.2 維持 DPR 1.5、PointLight 2 顆、零 transmission 與最終追逐幀 114/120 draw calls。
目前下一步是以 iOS／Android 真機完成觸控、視線、音效與長時間 frame-time 回歸，不再擴張 Door 3 玩法範圍。

```bash
npm run check      # 型別 + 相依分層 + 120 個單元測試
