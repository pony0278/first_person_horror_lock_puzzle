...
目前階段：**F2 門 3 壓力路由與爆管演出**（門 1、門 2 玩法已接通；門 2 成功後會在淹水十字泵房中心短暫停留，再走到偏左工作檯的正面操作位。門 3 已啟用守恆轉液、雙門閂目標與手動總閘；三向怪物仍刻意保持凍結）
...
門 3 已完成淹水十字泵房與壓力路由：Door 2 門後是 16m 積水連接廊，攝影機沿同一組世界座標奔跑約 19.62m／4.9 秒抵達泵房中心，原地停留 0.70 秒看清十字空間，再用 1.12 秒走到與工作檯中線對齊的正面操作位，鏡頭保持 0° 正視控制面板；開門即能看見遠方泵房，不切黑、不傳送。正面低矮工作檯使用 6／4／3 容量守恆轉液，五次標準路線可穩定左右門閂並解除手動總閘。第一次合法轉移會使後上方回水管爆裂：接頭水霧、弧形破碎水束與減壓細漏依序演出，落水點帶出飛濺、波紋與漂移雜物；玩家被淋則以約四秒、固定 seed、8 次取樣的濕視野折射表達，不再使用 CSS 泡泡。左／右／後仍以暖色粗管、冷色電纜、鐵鏈來區分未來威脅方向。Performance Stabilization v2.2 維持 DPR 1.5、PointLight 2 顆與零 transmission，濕視野的額外場景 pass 只在短時演出期間啟用；三向怪物規則仍未啟動。
目前下一步是先用真機確認 Door 3 的方向辨識、爆管回頭反應與濕視野強度，再接入誠實但不完整的三向怪物提示及嚴重錯誤推進規則。
