...
目前階段：**F2.3 門 3 誠實三向威脅**（門 1、門 2 玩法已接通；門 3 已啟用守恆轉液、雙門閂、首次爆管、實體穿門成功路徑與獨立三向威脅。左側水波、右側逐盞熄燈、後方鐵鏈會先提示同一隻怪物即將換道；隱藏牆鐘超時後死亡並整局重開）
...
| Job | 內容 | 擋發布嗎 |
| --- | --- | --- |
| **建置與發布** | `npm run check`（型別、相依分層、111 個單元測試）→ 建置單檔 → 產物大小門檻 → 發布 | 是 —— 但這些都是純 Node，兩秒跑完且完全確定性 |
...
- Door 2 正解提交、電磁鎖解開、開門、16m 積水長廊、十字中心揭露與工作檯操作位。
...
src/
  ...
  render/                 ── 場景建構與繪製 ──
    ...
    pumphub.js              門 3 十字泵房、防洪閘門、三向走廊與三缸液面
    pumpconsole.js           低矮偏左工作檯、六個 ± 控制鍵與即時壓力表
...
門 3 已完成淹水十字泵房、壓力路由、正式成功路徑與 F2.3 三向威脅：Door 2 門後是 16m 積水連接廊，攝影機沿同一組世界座標奔跑約 19.62m／4.9 秒抵達泵房中心，原地停留 0.70 秒看清十字空間，再用 1.12 秒走到工作檯正面。6／4／3 容量守恆轉液以五次標準路線穩定左右門閂；第一次合法轉移會使後上方回水管爆裂並觸發四秒濕視野。工作檯交還控制後啟動 38.3 秒隱藏牆鐘，同一隻怪物依後→左→右→後→左→右換道並逐段接近；後方鐵鏈、左側向內水波與右側由深到近熄燈必定先於怪物現身，非當前方向不製造同型假提示。已鎖定水缸被移出目標帶時會回撞並把威脅推到下一階段起點，仍重新播放新方向提示。最終右側全黑後保留 1.25 秒可見窗口才死亡，之後整局回到 Door 1；有效拉下總閘會凍結 F2.3 威脅，門片總成升起後玩家實際穿門並在安全側停留 2.4 秒才成功。Performance Stabilization v2.2 維持 DPR 1.5、PointLight 2 顆與零 transmission。
目前下一步是先補足橫／直向實際瀏覽器與真機畫面驗收，再進入 F2.4：凝視牽制、嚴重錯誤的近距離平衡，以及拉桿至穿門之間的最後追殺窗口。

```bash
npm run check      # 型別 + 相依分層 + 111 個單元測試
