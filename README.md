...
src/
  ...
  logic/                  ── 純邏輯，strict TypeScript，不碰 DOM 與 Three.js ──
    config.ts               調校參數與其型別
    lock.ts                 撞針鎖狀態機
    pin-puzzle.ts           門 1 圖形等差題、唯一違規圖形與答案推演
    pins.ts                 撞針狀態與難度變體型別
  ...
  render/                 ── 場景建構與繪製 ──
    ...
    hintwall.js             Rough.js＋SVG＋Canvas 側邊同款撞針圖形／刻點等差題、手電筒、暗角
...
門 1 已改為單來源圖形等差推理＋原撬鎖執行：牆上四個等距位置直接使用鎖內同款圖形，每個圖形帶 1～5 個刻點；只有移除假針圖形後，剩餘刻點數形成 1、3、5。剩餘圖形由左到右就是撬鎖順序，門上不再放第二套對照。
門 2 已完成首輪體驗調整：總時限仍為 20 秒，怪物站位改為 7／11／14／20 秒，空槽提供無文字缺件脈衝。
目前的下一步是用真機與小規模玩家驗收門 1 是否能一眼開始找圖形等差規律、五個刻點在側視距離是否可數、走廊恐怖構圖與門 2 壓力曲線，再進入門 3。
