...
目前階段：**F1 門 1＋門 2 垂直切片**（門 1 雙來源刮痕推理與撬鎖已接通；門 2 管線、20 秒追逐已接通；門 3 尚未實作）
...
src/
  ...
  logic/                  ── 純邏輯，strict TypeScript，不碰 DOM 與 Three.js ──
    config.ts               調校參數與其型別
    lock.ts                 撞針鎖狀態機
    pin-puzzle.ts           門 1 牆規律、門面刻痕映射與答案推演
    pins.ts                 撞針狀態與難度變體型別
  ...
  render/                 ── 場景建構與繪製 ──
    ...
    hintwall.js             Rough.js＋SVG＋Canvas 側邊 1／2／抹除位／4 刻痕序列、手電筒、暗角
    doorclue.js             門 1 四符號／1～4 道金屬刻痕對照
...
門 1 已改為雙來源缺格推理＋原撬鎖執行：牆上四個等距位置依序留下 1、2、遭焦黑刪除、4 道刻痕；門上四個撞針符號分別綁定 1～4 道刻痕。玩家補出第三位、排除三道刻痕對應的假針，再依 1→2→4 查門面完成撬鎖。
門 2 已完成首輪體驗調整：總時限仍為 20 秒，怪物站位改為 7／11／14／20 秒，空槽提供無文字缺件脈衝。
目前的下一步是用真機與小規模玩家驗收門 1 的跨視角理解率、門面刻痕可讀性、走廊恐怖構圖與門 2 壓力曲線，再進入門 3。
