...
目前階段：**F1 門 1＋門 2 垂直切片**（門 1 單來源圖形等差推理與撬鎖已接通；門 2 管線、20 秒追逐已接通；門 3 尚未實作）
...
src/
  ...
  logic/                  ── 純邏輯，strict TypeScript，不碰 DOM 與 Three.js ──
    config.ts               調校參數與其型別
    lock.ts                 撞針鎖狀態機
    pin-puzzle.ts           門 1 缺格位置序列、動態五點軌道與答案推演
    pins.ts                 撞針狀態與難度變體型別
  ...
  render/                 ── 場景建構與繪製 ──
    ...
    hintwall.js             Rough.js＋SVG＋Canvas 側邊缺格五點軌道、手電筒、暗角
...
門 1 已改為缺格位置序列＋原撬鎖執行：牆面以被拔走的中央軌道明示缺格，兩端同為五點軌道，亮點位於最左與最右（或反向）；門鎖四個候選包含最左、正中央、最右與一個偏心位置。玩家補回正中央後，依完整序列由左到右撬動三根真針，偏心位置是假針。
門 2 已完成首輪體驗調整：總時限仍為 20 秒，怪物站位改為 7／11／14／20 秒，空槽提供無文字缺件脈衝。
目前的下一步是用真機與小規模玩家驗收門 1 的中央缺格是否一眼可辨、五點軌道與偏心位置在側視／鎖面距離是否清楚、走廊恐怖構圖與門 2 壓力曲線，再進入門 3。
