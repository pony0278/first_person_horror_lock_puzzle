...
目前階段：**F2 門 3 場景與工作檯灰盒**（門 1、門 2 玩法已接通；門 2 成功後會在淹水十字泵房中心短暫停留，再走到偏左工作檯的正面操作位。門 3 三缸液面／壓力工作檯已可操作，正式水量目標與三向怪物仍刻意保持凍結）
...
門 3 已先完成淹水十字泵房灰盒：Door 2 門後是 16m 積水連接廊，攝影機沿同一組世界座標奔跑約 19.62m／4.9 秒抵達泵房中心，原地停留 0.70 秒看清十字空間，再用 1.12 秒走到與工作檯中線對齊的正面操作位，鏡頭保持 0° 正視控制面板；開門即能看見遠方泵房，不切黑、不傳送。舊 Door 1→2 前室只在這段過場停用，避免端牆遮擋與穿牆。正面是防洪閘門與三缸壓力組，入口偏左已裝設不阻擋中央通道的低矮工作檯；六個 ± 控制鍵可分別改變三缸液面，壓力表會依平均液面即時轉動。左／右／後分別以暖色粗管、冷色電纜、鐵鏈來區分方向；操作區收起後可用拖曳或 W/A/S/D 環視。Performance Stabilization v2.2 維持 DPR 1.5、PointLight 2 顆與零 transmission，並把肋架／燈管／鏈節改為 InstancedMesh、依材質合併靜態地板／天花板／牆面，再以 draw-call 預算與 rolling frame-time 探針攔截退化；這一輪已啟用手動液面／壓力操作，但仍未啟動正式解謎目標與怪物規則。
